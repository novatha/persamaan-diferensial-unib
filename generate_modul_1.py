...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textbf{Persamaan Diferensial 2026} -- Teknik Elektro Universitas Bengkulu}
\fancyhead[R]{Modul Ajar Minggu 1: Pengantar \& Klasifikasi PD}
\fancyfoot[C]{\thepage}

% Pengaturan kode GNU Octave
...
\textbf{\LARGE PENGANTAR PERSAMAAN DIFERENSIAL: KLASIFIKASI PDB/PDP, MASALAH NILAI AWAL (IVP), FISIKA DINAMIKA ENERGI RANGKAIAN ELEKTRIK \& KOMPUTASI GNU OCTAVE}}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{C6 (Menciptakan/Komputasi):} Mengembangkan skrip modular berbasis \textbf{GNU Octave} untuk mensimulasikan respon transien arus pengisian induktor, memvalidasi solusi eksak analitik terhadap solusi numerik integrasi Runge-Kutta (\texttt{ode45}), serta memvisualisasikan medan arah (\textit{slope field}).
\end{enumerate}
\end{tcolorbox}
...
Mata kuliah Persamaan Diferensial di Jurusan Teknik Elektro dirancang secara khusus untuk membangun landasan analitik dan intuisi fisika yang kokoh bagi dua pilar utama rekayasa elektro: **Analisis Transien Rangkaian Listrik** (pada paruh pertama semester) dan **Teori Medan Elektromagnetika \& Perambatan Gelombang** (pada paruh kedua semester).
...
\begin{tikzpicture}[
    ...
    block/.style={rectangle, draw=headercolor, fill=blue!8, thick, rounded corners=3pt, align=center, minimum width=3.3cm, minimum height=1.0cm, font=\scriptsize},
    highlight/.style={rectangle, draw=red!80!black, fill=red!15, very thick, rounded corners=3pt, align=center, minimum width=3.5cm, minimum height=1.1cm, font=\scriptsize\bfseries},
    midblock/.style={rectangle, draw=orange!80!black, fill=orange!15, thick, rounded corners=3pt, align=center, minimum width=3.3cm, minimum height=1.0cm, font=\scriptsize\bfseries},
    ...
    \node[block, left=of w13] (w14) {Minggu 14: Fungsi Bessel \&\\Efek Kulit (*Skin Effect*)};
...
\section{Pendahuluan \& Filosofi Dinamika Sistem dalam Rekayasa Elektro}
...
Hukum-hukum fundamental fisika tidak pernah menyatakan besaran fisik secara langsung, melainkan menyatakan **laju perubahan** (\textit{rate of change}) dari besaran tersebut:
\begin{enumerate}
    \item **Hukum Arus Kapasitor (Arus Perpindahan):**
    ...
    \item **Hukum Tegangan Induktor (Hukum Induksi Faraday):**
    ...
    \item **Hukum Kekekalan Energi pada Elemen Terpusat:**
...
Karena laju perubahan energi kinetik medan magnet ($\frac{dE_L}{dt}$) dan medan listrik ($\frac{dE_C}{dt}$) harus setimbang dengan daya yang dipasok oleh sumber dan diserap oleh resistor pada setiap sekon waktu, maka hubungan antar-komponen ini secara matematis **pasti menghasilkan persamaan diferensial**.

---

\section{Taksonomi \& Klasifikasi Persamaan Diferensial}

Untuk dapat menyelesaikan suatu persamaan diferensial dengan metode analitik maupun numerik yang tepat, seorang insinyur elektro harus mampu mengklasifikasikan persamaan tersebut berdasarkan empat atribut utama: **tipe**, **orde**, **derajat**, dan **linieritas**.
...
Persamaan Diferensial Biasa (PDB) adalah persamaan diferensial yang hanya memuat turunan dari satu atau lebih variabel terikat terhadap **satu variabel bebas tunggal**. Dalam analisis rangkaian listrik, variabel bebas ini umumnya adalah waktu $t$ (pada domain transien) atau posisi $x$ (pada saluran transmisi).
\end{definition}
...
Persamaan Diferensial Parsial (PDP) adalah persamaan diferensial yang memuat turunan parsial dari satu variabel terikat terhadap **dua atau lebih variabel bebas independen**.
\end{definition}
...
\end{equation}
Persamaan~\eqref{eq:telegrapher_full} dikenal sebagai **Persamaan Telegrafer** (\textit{Telegrapher's Equation}) yang diturunkan oleh Oliver Heaviside pada tahun 1876.

\subsection{Orde dan Derajat Persamaan Diferensial}

\begin{definition}[Orde Persamaan Diferensial]
Orde dari suatu persamaan diferensial ditentukan oleh **tingkat turunan tertinggi** yang muncul di dalam persamaan tersebut.
\end{definition}

\begin{definition}[Derajat Persamaan Diferensial]
Derajat dari suatu persamaan diferensial adalah **pangkat aljabar tertinggi** dari turunan tingkat tertinggi tersebut, setelah persamaan dibebaskan dari bentuk pecahan atau tanda akar pada turunan-turunannya.
\end{definition}
...
Sifat linieritas adalah sifat yang paling mendasar dalam rekayasa sistem. Suatu PDB dikatakan **linier** jika variabel terikat $y$ dan seluruh turunannya ($y', y'', \dots, y^{(n)}$) hanya berderajat satu dan tidak saling dikalikan satu sama lain, serta tidak berada di dalam fungsi transenden (seperti $\sin(y), \cos(y), e^y, \ln(y)$).
...
\end{equation}
Artinya, jika sistem dipasok oleh kombinasi sumber eksitasi $g(t) = c_1 g_1(t) + c_2 g_2(t)$, respon total sistem adalah penjumlahan langsung dari respon masing-masing sumber secara independen. Sifat ini **runtuh total** pada persamaan diferensial non-linier!
\end{tcolorbox}

\subsection{Homogenitas: Respons Alami vs Respons Paksa}
Pada Persamaan~\eqref{eq:linear_ode}:
\begin{itemize}
    \item Jika $g(t) = 0$, persamaan disebut **PDB Homogen**. Solusinya merepresentasikan **respons alami** (\textit{natural response} atau respons bebas) dari sistem, yaitu pelepasan energi yang tersimpan mula-mula di dalam induktor atau kapasitor tanpa ada catu daya luar.
    \item Jika $g(t) \neq 0$, persamaan disebut **PDB Non-Homogen**. Solusinya memuat gabungan respons alami dan **respons paksa** (\textit{forced response}), yang dipicu oleh sumber tegangan/arus luar $g(t)$.
\end{itemize}
...
Ketika kita mengintegrasikan suatu persamaan diferensial biasa orde ke-$n$, proses integrasi akan memunculkan $n$ buah konstanta sembarang ($C_1, C_2, \dots, C_n$). Solusi yang masih memuat konstanta-konstanta ini dinamakan **Solusi Umum** (\textit{General Solution}).

Secara geometris, solusi umum merepresentasikan **keluarga tak hingga dari kurva integral** di bidang koordinat $(t, y)$, di mana setiap kurva memiliki bentuk kelengkungan yang serupa namun bergeser posisinya bergantung pada nilai konstanta integrasi $C$.

\subsection{Medan Arah (\textit{Slope Field / Direction Field})}
Seringkali persamaan diferensial yang memodelkan sistem fisik nyata sangat sulit atau bahkan mustahil diselesaikan secara analitik tertutup. Namun, kita dapat memvisualisasikan seluruh perilaku dinamika sistem secara grafis menggunakan **Medan Arah** (\textit{Slope Field}).
...
Suatu persamaan diferensial orde $n$ yang dilengkapi dengan $n$ buah syarat awal pada satu titik waktu acuan yang sama ($t = t_0$) dinamakan **Masalah Nilai Awal** (\textit{Initial Value Problem} - IVP):
\begin{equation}
    \begin{cases}
...
Syarat-syarat awal ini berfungsi untuk menetapkan secara tunggal nilai dari konstanta sembarang $C_1, \dots, C_n$, sehingga dari keluarga tak hingga kurva solusi umum, terpilih tepat **satu kurva solusi khusus** yang secara presisi menggambarkan realitas fisis sistem yang diamati.
...
\subsection{Prinsip Fisika Kontinuitas Energi: Mengapa $i_L(0^-) = i_L(0^+)$ dan $v_C(0^-) = v_C(0^+)$?}
Bagi seorang insinyur elektro, kondisi awal pada saat sakelar ditutup ($t = 0^+$) bukan sekadar angka matematis sembarang, melainkan ditentukan secara mutlak oleh **Hukum Kekekalan Energi Alam Semesta**:

\begin{enumerate}
    \item **Kontinuitas Arus Induktor:**
    ...
    \end{equation}
    Jika arus induktor melompat secara diskontinu dari nilai $i_L(0^-)$ ke $i_L(0^+)$ dalam durasi waktu nol ($\Delta t \to 0$), maka laju perubahan arus bernilai tak hingga ($\frac{di_L}{dt} \to \infty$). Akibatnya, tegangan induktor $v_L(t)$ dan daya sesaat $p_L(t)$ harus bernilai **tak hingga ($\infty$ Watt)**!
    
    Karena tidak ada sumber daya di alam semesta yang memiliki daya tak hingga, maka **energi medan magnet tidak dapat berubah seketika**:
    \begin{equation}
        E_L(0^-) = E_L(0^+) \implies \frac{1}{2} L [i_L(0^-)]^2 = \frac{1}{2} L [i_L(0^+)]^2 \implies i_L(0^-) = i_L(0^+)
        \label{eq:continuity_inductor}
    \end{equation}
    
    \item **Kontinuitas Tegangan Kapasitor:**
    Dengan analogi yang sama, energi medan listrik pada kapasitor adalah $E_C = \frac{1}{2} C [v_C(t)]^2$. Agar daya pengisian tidak tak hingga, **tegangan pada pelat kapasitor tidak dapat berubah seketika**:
    \begin{equation}
        v_C(0^-) = v_C(0^+)
...
Mari kita turunkan formulasi matematis PDB dari diagram skematik rangkaian listrik secara *first-principles*.
...
Persamaan~\eqref{eq:ode_rl_standard} adalah **PDB Linier Orde 1 Non-Homogen Koefisien Konstan**.
...
\end{equation}
Persamaan~\eqref{eq:general_sol_rl} adalah **Solusi Umum** pengisian arus rangkaian RL.
...
Substitusikan kembali nilai $K$ ke dalam solusi umum untuk memperoleh **Solusi Khusus**:
\begin{equation}
...
\subsection{Dinamika Transien: Evaluasi Berbasis Kelipatan Konstanta Waktu $\tau$}
...
Selama setengah detik tersebut, inti besi transformator dapat mengalami kejenuhan (\textit{magnetic saturation}), memicu arus pemula transien (\textit{inrush current}) yang dapat mencapai **6 hingga 10 kali lipat arus nominal beban penuh**. Pemodelan persamaan diferensial transien ini menjadi rujukan wajib bagi insinyur proteksi PLN untuk menyetel relai diferensial transformator (*relay 87T*) agar tidak salah trip (*nuisance tripping*) saat gardu induk pertama kali diberi tegangan (*energized*).

---

\section{Contoh Soal Terhitung Langkah demi Langkah (*Worked Numerical Examples*)}
...
\begin{enumerate}[leftmargin=*]
    \item **Persamaan 1:**
    \begin{itemize}
        \item Variabel bebas tunggal ($t$) $\implies$ **PDB**.
        \item Turunan tertinggi adalah $\frac{d^2 i}{dt^2}$ $\implies$ **Orde 2**.
        \item Pangkat dari turunan tertinggi adalah satu $\implies$ **Derajat 1**.
        \item Koefisien adalah konstanta ($5, 20, 500$), variabel terikat $i$ dan seluruh turunannya berderajat 1 dan tidak ada perkalian antar-turunan $\implies$ **Linier** (Non-Homogen).
    \end{itemize}
    
    \item **Persamaan 2:**
    \begin{itemize}
        \item Dua variabel bebas spasial ($x, y$) $\implies$ **PDP** (Persamaan Poisson 2D).
        \item Turunan parsial tertinggi adalah turunan kedua $\implies$ **Orde 2**.
        \item Pangkat turunan parsial tertinggi adalah satu $\implies$ **Derajat 1**.
        \item Variabel terikat $V$ dan seluruh turunannya linier $\implies$ **Linier**.
    \end{itemize}
    
    \item **Persamaan 3:**
    \begin{itemize}
        \item Variabel bebas tunggal ($t$) $\implies$ **PDB**.
        \item Turunan tertinggi adalah $\frac{di}{dt}$ $\implies$ **Orde 1**, **Derajat 1**.
        \item Terdapat suku non-linier $i^2$ (kuadrat dari variabel terikat) $\implies$ **Non-Linier**. (Model resistor tak linier / varistor).
    \end{itemize}
    
    \item **Persamaan 4:**
    \begin{itemize}
        \item Variabel bebas tunggal ($t$) $\implies$ **PDB**.
        \item Turunan tertinggi adalah $\frac{d^3 y}{dt^3}$ $\implies$ **Orde 3**.
        \item Turunan tingkat ketiga berpangkat 2 $\implies$ **Derajat 2**.
        \item Karena derajat $>1$, persamaan ini otomatis **Non-Linier**.
    \end{itemize}
\end{enumerate}
\end{example}
...
\section{Praktikum Komputasi Matematika Terbuka dengan GNU Octave}

Untuk memverifikasi solusi analitik dan mengamati dinamika transien secara visual, mahasiswa wajib menjalankan skrip modular berbasis **GNU Octave** berikut.

\begin{lstlisting}[style=octave, caption={Skrip GNU Octave: Simulasi Transien Rangkaian RL dan Verifikasi Numerik ODE45.}]
% =========================================================================
% MODUL AJAR PERSAMAAN DIFERENSIAL - JURUSAN TEKNIK ELEKTRO UNIB
% Skrip Praktikum Minggu 1: Respon Transien Rangkaian RL (Analitik vs Numerik)
...
V0 = 125.0;         % Tegangan Sumber DC (Volt)
R  = 2.5;           % Resistansi Kumparan (Ohm)
L  = 0.050;         % Induktansi Kumparan (Henry)
tau = L / R;        % Konstanta Waktu (detik)
Iss = V0 / R;       % Arus Keadaan Mantap (Ampere)

fprintf('--- PARAMETER RANGKAIAN RL ---\n');
fprintf('Tegangan Sumber (V0) : %.1f Volt\n', V0);
fprintf('Resistansi (R)       : %.2f Ohm\n', R);
fprintf('Induktansi (L)       : %.3f Henry\n', L);
fprintf('Konstanta Waktu (tau): %.3f detik (%.1f ms)\n', tau, tau*1000);
fprintf('Arus Mantap (Iss)    : %.1f Ampere\n', Iss);

% 2. Vektor Waktu Simulasi (0 s.d. 6 tau)
t_end = 6 * tau;
t_exact = linspace(0, t_end, 500);

% 3. Solusi Eksak Analitik
i_exact = Iss * (1 - exp(-t_exact / tau));
v_induktor_exact = V0 * exp(-t_exact / tau);
daya_resistor = (i_exact.^2) * R;

% 4. Solusi Numerik Menggunakan ODE45 (Metode Runge-Kutta Orde 4-5)
% PDB: di/dt = (V0 - R*i) / L
f_ode = @(t, i) (V0 - R * i) / L;
i_initial = 0.0;     % Kondisi Awal i(0) = 0
[t_num, i_num] = ode45(f_ode, [0, t_end], i_initial);

% 5. Evaluasi Selisih Error Mutlak (Validasi Numerik)
i_exact_interp = interp1(t_exact, i_exact, t_num);
error_abs = abs(i_exact_interp - i_num);
max_error = max(error_abs);
fprintf('Maksimum Error Mutlak Numerik vs Analitik: %.2e Ampere\n', max_error);

% 6. Visualisasi Grafik Berkualitas Tinggi
figure('Name', 'Respon Transien Rangkaian RL', 'NumberTitle', 'off', 'Position', [100, 100, 900, 600]);

subplot(2, 1, 1);
plot(t_exact*1000, i_exact, 'b-', 'LineWidth', 2.2, 'DisplayName', 'Solusi Analitik Eksak');
hold on;
plot(t_num*1000, i_num, 'ro', 'MarkerSize', 4, 'DisplayName', 'Solusi Numerik ODE45');
yline(Iss, 'k--', 'LineWidth', 1.2, 'DisplayName', sprintf('Keadaan Mantap (Iss = %.0f A)', Iss));
xline(tau*1000, 'g--', 'LineWidth', 1.2, 'DisplayName', sprintf('1\\tau = %.1f ms (63.2%%)', tau*1000));
xline(5*tau*1000, 'm--', 'LineWidth', 1.2, 'DisplayName', sprintf('5\\tau = %.1f ms (99.3%%)', 5*tau*1000));
grid on;
title('Respon Transien Pengisian Arus Induktor: i(t) vs Waktu');
xlabel('Waktu (milidetik)');
ylabel('Arus Induktor i(t) [Ampere]');
legend('Location', 'southeast');
ylim([0, Iss * 1.15]);

subplot(2, 1, 2);
plot(t_exact*1000, v_induktor_exact, 'r-', 'LineWidth', 2.0, 'DisplayName', 'Tegangan Induktor v_L(t)');
hold on;
plot(t_exact*1000, daya_resistor/1000, 'k-', 'LineWidth', 2.0, 'DisplayName', 'Disipasi Daya Resistor P_R(t) [kW]');
grid on;
title('Dinamika Tegangan Induktor dan Disipasi Daya Termal');
xlabel('Waktu (milidetik)');
ylabel('v_L (Volt) / Daya (kW)');
legend('Location', 'northeast');

fprintf('Simulasi GNU Octave selesai dengan sukses!\n');
\end{lstlisting}
...
\begin{enumerate}[leftmargin=*]
    ...
    Jelaskan mengapa pada saat sakelar dihubungkan ke sumber tegangan secara mendadak, sebuah induktor yang awalnya kosong bertindak sebagai **rangkaian terbuka** (\textit{open circuit}), sedangkan kapasitor yang awalnya kosong bertindak sebagai **hubung singkat** (\textit{short circuit}). Buktikan dengan meninjau rumus $v_L = L \frac{di}{dt}$ dan $i_C = C \frac{dv}{dt}$.
...
\begin{itemize}
    \item **Persamaan Diferensial** adalah formulasi matematika tak tergantikan untuk mendeskripsikan dinamika laju perubahan energi, muatan, dan fluks dalam rekayasa elektro.
    \item **PDB** hanya memuat satu variabel bebas (biasanya waktu $t$), sedangkan **PDP** memuat dua atau lebih variabel bebas (seperti waktu $t$ dan ruang $x, y, z$).
    \item **Linieritas** menjamin berlakunya prinsip superposisi, yang memungkinkan analisis respon total sistem kompleks sebagai penjumlahan dari respon komponen-komponennya.
    \item **Masalah Nilai Awal (IVP)** menetapkan nilai konstanta integrasi sembarang $C$ dari solusi umum menjadi kurva solusi khusus tunggal yang merefleksikan kontinuitas energi fisik sistem ($i_L(0^-) = i_L(0^+)$ dan $v_C(0^-) = v_C(0^+)$).
    \item **Konstanta Waktu $\tau = L/R$** merepresentasikan skala waktu alami transien pengisian/pengosongan induktor, di mana pada $t = 5\tau$, respon sistem telah mencapai $>99\%$ dari keadaan mantap (*steady state*).
\end{itemize}
...
with open("modul_1.tex", "w") as f:
    f.write(tex_content)

print("modul_1.tex generated. Compiling with pdflatex...")
res = subprocess.run(["pdflatex", "-interaction=nonstopmode", "modul_1.tex"], capture_output=True, text=True)
print("Return code:", res.returncode)
if res.returncode == 0:
    res2 = subprocess.run(["pdflatex", "-interaction=nonstopmode", "modul_1.tex"], capture_output=True, text=True)
    print("Pass 2 return code:", res2.returncode)
    print("PDF generation success!")
else:
    print("Errors:")
    print("\n".join([line for line in res.stdout.splitlines() if "!" in line or "Error" in line][:20]))
