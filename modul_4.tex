...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{Persamaan Diferensial - Teknik Elektro UNIB}}
\fancyhead[R]{\textit{Minggu 4}}
...
\title{\textbf{Modul Pembelajaran Persamaan Diferensial}\\\large Minggu 4: Persamaan Diferensial Biasa Orde 2 Linier Homogen Koefisien Konstan}
\author{\textbf{Novalio Daratha, Ph.D.} \& \textbf{Muhammad Arfan, M.T.}\\Program Studi Teknik Elektro, Fakultas Teknik, Universitas Bengkulu}
\date{Tahun Akademik 2026}
...
\begin{enumerate}[noitemsep]
    \item Menjelaskan struktur PDB linier orde 2 homogen dengan koefisien konstan.
    \item Menguji kebebasan linier pasangan solusi basis menggunakan determinan Wronskian.
    \item Membuktikan kemunculan suku $x e^{rx}$ pada akar kembar melalui Metode Reduksi Orde.
    \item Menyelesaikan Masalah Nilai Awal (IVP) untuk 3 kasus diskriminan ($D > 0, D = 0, D < 0$).
    \item Memodelkan osilasi alami pada sistem fisik rangkaian listrik LC murni tanpa redaman.
\end{enumerate}
\end{tcolorbox}
...
\section{Pendahuluan: Sistem Orde Dua dalam Rekayasa Elektro}
...
\begin{itemize}
    \item \textbf{Induktor ($L$):} menyimpan energi dalam bentuk medan magnetik ($E_L = \frac{1}{2} L i^2$).
    \item \textbf{Kapasitor ($C$):} menyimpan energi dalam bentuk medan listrik ($E_C = \frac{1}{2} C v^2 = \frac{q^2}{2C}$).
\end{itemize}
...
\subsection{Uji Kebebasan Linier: Determinan Wronskian}
...
\begin{itemize}
    ...
    \item Jika $W(y_1, y_2) = 0$, kedua fungsi bergantung linier dan tidak dapat membentuk solusi umum.
\end{itemize}

---

\section{Persamaan Karakteristik \& Tiga Ragam Solusi}
...
\subsection{Kasus 1: Diskriminan Positif ($D = b^2 - 4ac > 0$)}
...
\end{equation}
\textit{Interpretasi Fisis:} Bersesuaian dengan sistem tanpa osilasi yang kembali ke titik kesetimbangan secara lambat (*Overdamped*).
\end{tcolorbox}

\begin{example}[Masalah Nilai Awal Kasus 1]
Selesaikan Masalah Nilai Awal (IVP) berikut:
$$ y'' - y' - 6y = 0, \quad y(0) = 5, \quad y'(0) = 0 $$
\textbf{Langkah Penyelesaian:}
\begin{enumerate}
    \item \textbf{Susun Persamaan Karakteristik:}
    $$ r^2 - r - 6 = 0 \implies (r - 3)(r + 2) = 0 \implies r_1 = 3, \quad r_2 = -2 $$
    \item \textbf{Tuliskan Solusi Umum:}
    $$ y(x) = C_1 e^{3x} + C_2 e^{-2x} $$
    \item \textbf{Hitung Turunan Pertama:}
    $$ y'(x) = 3C_1 e^{3x} - 2C_2 e^{-2x} $$
    \item \textbf{Terapkan Kondisi Awal:}
    $$ y(0) = C_1 + C_2 = 5 $$
    $$ y'(0) = 3C_1 - 2C_2 = 0 \implies C_2 = \frac{3}{2} C_1 $$
    Substitusi ke persamaan pertama:
    $$ C_1 + \frac{3}{2} C_1 = 5 \implies \frac{5}{2} C_1 = 5 \implies C_1 = 2 \implies C_2 = 3 $$
    \item \textbf{Solusi Khusus:}
...
\subsection{Kasus 2: Diskriminan Nol ($D = b^2 - 4ac = 0$)}
...
\begin{enumerate}
    \item Karena $r = -\frac{b}{2a}$, maka $2ar + b = 2a\left(-\frac{b}{2a}\right) + b = 0$.
    \item Karena $r$ adalah akar karakteristik, maka $a r^2 + b r + c = 0$.
\end{enumerate}
...
\end{equation}
\textit{Interpretasi Fisis:} Bersesuaian dengan sistem redaman kritis (*Critically Damped*) yang kembali ke titik netral paling cepat tanpa pernah berosilasi.
\end{tcolorbox}

\begin{example}[Masalah Nilai Awal Kasus 2]
Selesaikan IVP: $y'' + 6y' + 9y = 0$, dengan $y(0) = 2$ dan $y'(0) = -1$.
\begin{enumerate}
    \item Persamaan karakteristik: $r^2 + 6r + 9 = (r + 3)^2 = 0 \implies r = -3$ (akar kembar).
    \item Solusi umum: $y(x) = (C_1 + C_2 x)e^{-3x}$.
    \item Turunan: $y'(x) = C_2 e^{-3x} - 3(C_1 + C_2 x)e^{-3x}$.
    \item Kondisi awal:
    $$ y(0) = C_1 = 2 $$
    $$ y'(0) = C_2 - 3C_1 = -1 \implies C_2 - 3(2) = -1 \implies C_2 = 5 $$
    \item Solusi khusus: $\mathbf{y(x) = (2 + 5x)e^{-3x}}$.
\end{enumerate}
\end{example}

---

\subsection{Kasus 3: Diskriminan Negatif ($D = b^2 - 4ac < 0$)}
...
\end{equation}
\textit{Interpretasi Fisis:} Bersesuaian dengan sistem osilasi teredam (*Underdamped*), di mana $\alpha$ mengatur laju peluruhan amplutido dan $\beta$ mengatur frekuensi osilasi sudut.
\end{tcolorbox}

\begin{example}[Masalah Nilai Awal Kasus 3]
Tentukan solusi dari: $y'' + 4y' + 13y = 0$, $y(0) = 3$, $y'(0) = 2$.
\begin{enumerate}
    \item Karakteristik: $r^2 + 4r + 13 = 0 \implies r = \frac{-4 \pm \sqrt{16 - 52}}{2} = \frac{-4 \pm \sqrt{-36}}{2} = -2 \pm 3j$.
    \item Parameter: $\alpha = -2, \beta = 3$.
    \item Solusi umum: $y(x) = e^{-2x}(C_1 \cos 3x + C_2 \sin 3x)$.
    \item Turunan:
    ...
    \item Solusi khusus: $\mathbf{y(x) = e^{-2x}\left( 3\cos 3x + \frac{8}{3}\sin 3x \right)}$.
\end{enumerate}
\end{example}
...
\subsection{Penurunan Model dari Hukum Kirchhoff (KVL)}
...
\subsection{Solusi Muatan dan Arus Listrik}
...
\subsection{Kekekalan Energi Total Rangkaian}
...
\section{Rangkuman Komparatif}
\begin{table}[h!]
\centering
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Kriteria} & \textbf{Kasus 1 ($D > 0$)} & \textbf{Kasus 2 ($D = 0$)} & \textbf{Kasus 3 ($D < 0$)} \\ \midrule
\textbf{Akar Karakteristik} & Real Berbeda ($r_1 \neq r_2$) & Real Kembar ($r_1 = r_2 = r$) & Kompleks Konjugat ($\alpha \pm j\beta$) \\
\textbf{Solusi Basis} & $e^{r_1 x}, e^{r_2 x}$ & $e^{rx}, x e^{rx}$ & $e^{\alpha x}\cos\beta x, e^{\alpha x}\sin\beta x$ \\
\textbf{Bentuk Solusi} & $y = C_1 e^{r_1 x} + C_2 e^{r_2 x}$ & $y = (C_1 + C_2 x)e^{rx}$ & $y = e^{\alpha x}(C_1 \cos\beta x + C_2 \sin\beta x)$ \\
\textbf{Rezim Fisik} & *Overdamped* (Sangat Redam) & *Critically Damped* (Redam Kritis) & *Underdamped* (Kurang Redam) \\
\textbf{Osilasi} & Tidak ada osilasi & Tidak ada osilasi & Ada osilasi sinusoidal \\
\textbf{Kecepatan Redaman} & Paling lambat (ekivalen $R$ besar) & Tercepat mencapai nol & Meluruh perlahan jika $\alpha < 0$ \\ \bottomrule
\end{tabular}
\end{table}
...
\section{Referensi}
\begin{enumerate}
    \item Erwin Kreyszig, \textit{Advanced Engineering Mathematics}, 10th Edition, John Wiley \& Sons, 2011.
    ...
    \item Charles K. Alexander \& Matthew N. O. Sadiku, \textit{Fundamentals of Electric Circuits}, 7th Edition, McGraw-Hill, 2021.
\end{enumerate}
